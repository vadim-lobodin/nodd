...
- **Variants** — authors declare alternative versions of a screen/component in code via the `useVariant()` hook or the `<Variant>` wrapper (`src/provider/variants/`); reviewers switch between them from a Variants panel in the overlay (toolbar button or `V`). Selection is per-viewer, persisted to `localStorage`, not synced. Variants ride the state-aware-comment machinery: `<Variant>` wraps the active option in `<NoddState name="{name}:{option}">`, so a comment placed on one option is tagged, hidden when another option is shown, and reachable (with "Show me" to flip back) from the sidebar — no schema change. Syncing the selection between viewers, share-a-combination deep links, and screenshot diffing are deferred. See `src/provider/variants/README.md`.
